...
func createStopsIndexes() {
	...
	// Stops staging
	stopsStagingCollection := GetCollection("stops_staging")
	stopsStagingIndex := []mongo.IndexModel{
		{
			Keys: bson.D{{Key: "primaryidentifier", Value: 1}},
		},
		{
	...
	opts = options.CreateIndexes()
	_, err = stopsStagingCollection.Indexes().CreateMany(context.Background(), stopsStagingIndex, opts)
	if err != nil {
		log.Error().Err(err).Msg("Creating Index")
	}
...
func createJourneysIndexes() {
	...
	journeyBlockDatasetDepartureIndexName := "JourneyBlockDatasetDeparture"
	_, err = journeysCollection.Indexes().CreateMany(context.Background(), []mongo.IndexModel{
		...
		{
			Options: &options.IndexOptions{
				Name: &journeyBlockDatasetDepartureIndexName,
			},
			Keys: bson.D{
