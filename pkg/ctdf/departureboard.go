...
func GenerateDepartureBoardFromJourneys(journeys []*Journey, stopRefs []string, dateTime time.Time, doEstimates bool) []*DepartureBoard {
	...
	// PERF(low-risk): Build a set of stopRefs once instead of repeatedly calling
	// slices.Contains(stopRefs, ...) (which is O(n)) inside the per-journey path loops below.
	stopRefsSet := make(map[string]struct{}, len(stopRefs))
	for _, stopRef := range stopRefs {
		stopRefsSet[stopRef] = struct{}{}
	}

	// PERF(high-risk): Batch the per-journey realtime_journeys FindOne (previously one
	...
	// PERF(low-risk): The per-journey realtime/block/destination Mongo round-trips are now
	// batched or remain serialised cheaply, but the 45-minute estimate branch and
	// destination-display fallback can still perform DB calls, so 200 goroutines are retained
	// to keep those overlapping. Capped at len(journeys) to avoid spawning idle goroutines.
	maxGoroutines := 200
	...
	for _, journey := range journeys {
		p.Go(func() *DepartureBoard {
			...
			if journey.Availability.MatchDate(dateTime) {
				// PERF(medium-risk, NOT APPLIED): This 4-hour-cutoff loop and the detail-extraction
				...
				for _, path := range journey.Path {
					// PERF(low-risk): map lookup instead of slices.Contains(stopRefs, ...)
				...
				// PERF(high-risk): Replaces journey.GetRealtimeJourney(...)'s per-journey FindOne
				// with a lookup into the batched map built above. The IsActive() gate is replicated
				// exactly (same method, same side-effects) so the resulting journey.RealtimeJourney
				...
				// PERF(high-risk, NOT APPLIED): This block does a Find (block journeys by
				...
				// PERF(high-risk, NOT APPLIED): GetDestinationStop() and GetService() each issue a
	...
	// PERF(low-risk): preallocate to the maximum possible length to avoid repeated slice growth.
